...
def _chunk(plaintext, key=KEY, file_id="fid", part=1, parts=1):
...
def test_decrypt_chunk_roundtrip():
    assert decrypt_chunk(KEY, _chunk(b"hello world"), "fid", 1, 1) == b"hello world"
...
def test_decrypt_chunk_wrong_part_fails_auth():
    ...
    blob = _chunk(b"data", part=1, parts=2)
    with pytest.raises(DecryptionError):
        decrypt_chunk(KEY, blob, "fid", 2, 2)


def test_decrypt_chunk_wrong_total_fails_auth():
    # The AAD binds the total chunk count too, so a chunk can't be replayed into
    # a file with a different number of parts.
    blob = _chunk(b"data", part=1, parts=3)
    with pytest.raises(DecryptionError):
        decrypt_chunk(KEY, blob, "fid", 1, 2)


def test_decrypt_chunk_wrong_file_id_fails_auth():
    blob = _chunk(b"data", file_id="fid")
    with pytest.raises(DecryptionError):
        decrypt_chunk(KEY, blob, "other", 1, 1)


def test_decrypt_chunk_wrong_key_fails():
    with pytest.raises(DecryptionError):
        decrypt_chunk(b"\x00" * 32, _chunk(b"data"), "fid", 1, 1)
