...
```jsonc
{
  ...
  "encryption": {
    ...
    "chunk_size": 65536,        // PLAINTEXT bytes per chunk; 1 .. 16777216 (16 MiB)
...
- **`chunk_size` is the plaintext size**, not the stored size; the stored chunk
  is `chunk_size + 28` bytes (IV + tag) except the last. Keep it ≤ 16 MiB — the
  worker buffers one whole chunk in memory while decrypting.
